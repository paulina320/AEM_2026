...
bool MAX11300::setPinMode(uint8_t pin, pinMode_t mode, uint8_t differentialPin) {
	...
	switch (mode) {
		...
		case analogIn:
			...
			configuration = (configuration & ~(MAX_FUNCPRM_RANGE_MASK)) | MAX_ADC_RANGE_0_2_5;
			break;
		case analogOut:
			configuration = (configuration & ~(MAX_FUNCID_MASK)) | MAX_FUNCID_DAC;
			configuration = (configuration & ~(MAX_FUNCPRM_RANGE_MASK)) | MAX_DAC_RANGE_0_10;
			break;
		case analogDifferential:
			...
			negConf = (negConf & ~(MAX_FUNCPRM_RANGE_MASK)) | MAX_ADC_RANGE_0_2_5;
...
bool MAX11300::setPinADCrange (uint8_t pin, ADCRange_t range) {
	return readModifyWriteRegister((MAX_FUNC_BASE + pin), MAX_FUNCPRM_RANGE_MASK, (uint16_t)(range));
}

ADCRange_t MAX11300::getPinADCrange (uint8_t pin) {
	uint16_t reg = readRegister(MAX_FUNC_BASE + pin);
	reg &= MAX_FUNCPRM_RANGE_MASK;
	if (reg == MAX_ADC_RANGE_0_10) return ADCZeroTo10;
	if (reg == MAX_ADC_RANGE_5_5) return ADCNegative5to5;
	if (reg == MAX_ADC_RANGE_10_0) return ADCNegative10to0;
	if (reg == MAX_ADC_RANGE_0_2_5) return ADCZeroTo2_5;
	return ADCrangeNONE;
}
...
bool MAX11300::setPinDACrange (uint8_t pin, DACRange_t range) {
	return readModifyWriteRegister((MAX_FUNC_BASE + pin), MAX_FUNCPRM_RANGE_MASK, (uint16_t)(range));
}

DACRange_t MAX11300::getPinDACrange (uint8_t pin) {
	uint16_t reg = readRegister(MAX_FUNC_BASE + pin);
	reg &= MAX_FUNCPRM_RANGE_MASK;
	if (reg == MAX_DAC_RANGE_0_10) return DACZeroTo10;
	if (reg == MAX_DAC_RANGE_5_5) return DACNegative5to5;
	if (reg == MAX_DAC_RANGE_10_0) return DACNegative10to0;
	return DACrangeNONE;
}
...
uint32_t MAX11300::getConversionRateSps(void) {
	switch (getConversionRate()) {
		case rate200ksps:
			return 200000UL;
		case rate250ksps:
			return 250000UL;
...
float MAX11300::estimateAnalogSampleRate(uint8_t activeAnalogPins, uint8_t averagingSamples) {
	if (activeAnalogPins == 0 || averagingSamples == 0) return 0.0f;
	return ((float)getConversionRateSps()) / ((float)activeAnalogPins * (float)averagingSamples);
}
...
uint16_t MAX11300::readRegister (uint8_t address, uint16_t * values, uint8_t size) {
	...
	_spi->transfer((address << 1) | 1);
	for (uint8_t i = 0; i < size; i++) {
		values[i] = ((uint16_t)(_spi->transfer(0x00)) << 8) | _spi->transfer(0x00);
	}
	digitalWrite(_select, HIGH);
	_spi->endTransaction();
	return size;
