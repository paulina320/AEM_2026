...
class MAX11300 {
	...
	public:
		...
		bool setPinADCrange (uint8_t pin, ADCRange_t range);
		ADCRange_t getPinADCrange (uint8_t pin);
		bool setPinADCref (uint8_t pin, ADCref_t reference);
		ADCref_t getPinADCref (uint8_t pin);
		bool setPinDACrange (uint8_t pin, DACRange_t range);
		DACRange_t getPinDACrange (uint8_t pin);
		...
		uint32_t getConversionRateSps(void);
		float estimateAnalogSampleRate(uint8_t activeAnalogPins, uint8_t averagingSamples = 1);
